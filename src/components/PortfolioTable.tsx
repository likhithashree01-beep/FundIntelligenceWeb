...
const rowHighlight = (flags: string[]): string => {
  if (flags.includes('at-risk')) return 'bg-rose-50/40';
  if (flags.includes('watch')) return 'bg-amber-50/40';
  return '';
};
              ...
              const moicClass =
                moic >= 1.5
                  ...
                  : moic >= 1
                    ...
                    : 'text-rose-700';
              return (
                <tr key={c.id} className={`${rowHighlight(c.flags)} hover:bg-slate-50/60`}>
                  <td className="whitespace-nowrap px-4 py-3 font-medium text-slate-900">
                    {c.name}
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 text-slate-600">{c.sector}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-slate-600">{c.country}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-right text-slate-700 tabular-nums">
                    {formatCurrencyCompact(c.revenue)}
                  </td>
                  <td
                    className={`whitespace-nowrap px-4 py-3 text-right tabular-nums ${c.ebitda < 0 ? 'text-rose-700' : 'text-slate-700'}`}
                  >
                    {formatCurrencyCompact(c.ebitda)}
                  </td>
                  <td
                    className={`whitespace-nowrap px-4 py-3 text-right tabular-nums ${c.ebitdaMargin < 0 ? 'text-rose-700' : 'text-slate-700'}`}
                  >
